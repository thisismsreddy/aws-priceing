...
def _pricing(df: pd.DataFrame):
    ...
    inst=df[(df['Product Family'].str.lower()=='compute instance')&(df['Region Code'].str.lower()=='us-east-1')&(df['Unit']=='Hrs')&(df['Currency']=='USD')&(df['Tenancy'].fillna('Shared')=='Shared')&(df['Operating System'].fillna('Linux')=='Linux')&(df['Price Per Unit']>0)]
    spec={itype:(int(g.iloc[0]['Vcpu']),int(float(str(g.iloc[0]['Memory']).split()[0]))) for itype,g in inst.groupby('Instance Type')}
    od=inst[inst['Term Type']=='OnDemand'].groupby('Instance Type')['Price Per Unit'].min().to_dict()
    ri=inst[(inst['Term Type']=='Reserved')&(inst['Lease Contract Length'].str.startswith('3'))&(inst['Purchase Option']=='No Upfront')].groupby('Instance Type')['Price Per Unit'].min().to_dict()
    stor=df.copy();_norm(stor)
    gp3= pd.to_numeric(stor[(stor['Product Family'].str.lower()=='storage')&(stor['Volume Api Name']=='gp3')&(stor['Region Code'].str.lower()=='us-east-1')&(stor['Unit'].str.contains('GB'))&(stor['Term Type']=='OnDemand')]['Price Per Unit']).min()/HOURS_IN_MONTH
